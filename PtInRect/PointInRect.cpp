	...
	switch(iMessage){
		...
		case WM_TIMER:
			...
			/*
				...
				이때 B벡터가 되기 위해 A벡터가 어느 방향으로 이동하였는지 판별하고 싶다면
				외적의 결과를 불러오면 된다.

				원점(O)으로부터 벡터 A와 벡터 B 즉, OA, OB에 대한 벡터를 만들고 외적 연산을 해주면 이 결과값을 보고
				방향을 판단할 수 있다는 것이다.

				아마  작성자와 같은 초보자라면 외적 연산을 수행할 때 어떤 벡터끼리 연산해야할지
				감이 잡히지 않을 것이다.

				위에서 말한대로 외적의 결과는 방향을 나타낸다는 것만 떠올리면 의외로 쉽게 이해할 수 있다.
				연산의 순서는 중요하지만 점의 순서따위는 중요하지 않다는 점을 기억하자.

				OA라는 벡터와 마우스 벡터를 외적 연산했다고 해보자.
				그리고 마우스 벡터와 OB라는 벡터 역시 연산했다고 생각해보자.

				이때 연산에 사용된 각각의 벡터와 결과값(말 그대로 특정 수치값)은 중요하지 않으며
				앞서 얘기했듯 OA와 마우스, 마우스와 OB 벡터가 전부 같은 방향인지를 판단하는 것이다.

				내 짧은 식견으로는 현재까지 컴퓨터 그래픽을 다루는데에서 외적은 그저 방향을 나타낸다는 것에
				의미를 두고 공부하면 될 것 같다.

				3차원으로 넘어가면 달라질 수 있으나 아직 2차원에 대한 이해도가 충분하지 않으므로
				시간을 갖고 생각해봐야 한다.

				참고로 가장 중요한 월드 공간, 로컬 공간 / 왼손, 오른손 좌표계에 대한 개념도 알고있어야 하는데
				그 범위는 어떻게 되는지, 좌표계를 변환하는 방법에 대해서도 숙지하고 있어야 한다.

				WinAPI의 좌표계는 제 4사분면에서 Y축의 부호가 반전된 것이므로 이를 유념하여 결과를 관찰해보기 바란다.

				나중엔 실수 좌표계 즉, 데카르트 좌표를 이용하여 스크린 좌표로 변환하는 방법에 대해서도
				알아볼 것이다.
			 */

			if(CrossResult[0] >= 0 && CrossResult[1] >= 0 && CrossResult[2] >= 0)
			{
				// 삼각형을 이쁘게 칠해주는 GDI 함수
				// 신경쓸 필요 없다.
